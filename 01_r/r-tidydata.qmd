...
Note that here we had to use the "`" symbol around the final column name because the column name includes a space. If there are no spaces in your column names, you would just write out their names with no quotes or apostrophes. We'll discuss more about proper column names [later in the session](#renaming-columns).
:::
...
knitr::kable(species_table, format = "html", caption = "N Penguins by Species") %>%
  ...
  kableExtra::save_kable("species_table.html")
```
...
Now let's use GitHub Desktop to commit these files and push them to our GitHub repository. You should see all of the files that have been modified since we cloned our repository. From the first [introductory session](r-intro.qmd), there should a file created by RStudio for your project, your ongoing "penguins.R" script, and your new "penguins_data.csv" data file. From this session, you should also see your new cleaned data files (one ".csv" and one ".rda") and your exported HTML table. Make sure the checkboxes are checked for these six files, write a succinct commit summary, then click the <kbd>Commit</kbd> button, then the <kbd>Push</kbd> button.
