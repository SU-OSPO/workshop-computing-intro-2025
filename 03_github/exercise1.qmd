...
1.  Select "Squash and merge". This will change the blue button to read "Squash and merge". Click that to do the squash and merge:
    ...
    ![](figures/squash_merge_main_bkup.png)
...
**When do you use squash and merge vs merge?** It's up to you, as each has its pros and cons:
...
Personally, most times I use Squash and Merge to keep the history on `main` clean and straight forward (this is particularly nice on GitHub when using pull requests, as it will refer to the PR in the commit message; we will see this in exercise 3). But when working with someone else on a branch I will sometimes use Merge instead.
